...
- `src/services/groqService.ts`: Browser-side assistant request wrapper that calls `/api/ask-artist`.
- `src/types/music.ts`: Shared music-related types.
- `src/utils/time.ts`: Time formatting helper.
- `api/ask-artist.js`: Vercel serverless route for Groq/Llama calls and server-only `GROQ_API_KEY` access.
